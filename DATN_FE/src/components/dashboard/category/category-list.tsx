...
const CategoryList = ({ categories, pages, currentPage }: IProps) => {
  ...
  const handleSearch = (event: FormEvent) => {
    ...
    let url: string = `/dashboard/category`;

    if (searchText !== "" && searchText.length > 0) {
      url = `/dashboard/category?searchText=${searchText}`;
    }
  ...
  return (
    <div className="bg-gray-800 p-5 rounded-lg mt-5">
      ...
      {categories.length == 0 && (
        <AdminNotFoundPage
          title="Not found"
          content="Không tìm thấy danh mục sản phẩm đang tìm kiếm"
        />
      )}
      {categories.length > 0 && (
        <>
              ...
              {categories.map((category: ICategory, index) => (
                <tr key={category.id} className="border-b border-gray-700">
                  ...
                  <td className="px-4 py-2">
                    <TagFiled
                      cssClass={
                        category.isActive ? "bg-lime-900" : "bg-red-700"
                      }
                      context={
                        category.isActive ? "Hoạt động" : "Ngưng hoạt động"
                      }
                    />
                  </td>
                  <td className="px-4 py-2">
