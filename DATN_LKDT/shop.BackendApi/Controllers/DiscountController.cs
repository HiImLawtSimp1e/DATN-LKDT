...
namespace shop.BackendApi.Controllers
{
    ...
    public class DiscountController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<Pagination<List<DiscountEntity>>>>> SearchAdminVouchers(string searchText, [FromQuery] int page, [FromQuery] double pageResults)
        {
            if (page == null || page <= 0)
            {
                page = 1;
            ...
            var response = await _service.SearchVouchers(searchText, page, pageResults);
            if (!response.Success)
            {
