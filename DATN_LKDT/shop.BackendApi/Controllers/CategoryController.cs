...
namespace shop.BackendApi.Controllers
{
    ...
    public class CategoryController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<Pagination<List<Product>>>>> SearchAdminCategories(string searchText, [FromQuery] int page, [FromQuery] double pageResults)
        {
            if (page == null || page <= 0)
            {
                page = 1;
            ...
            var response = await _service.SearchAdminCategories(searchText, page, pageResults);
            if (!response.Success)
            {
                return BadRequest(response);
            }
